...
public:
    ...
    nb::dict to_numpy(nb::object columns = nb::none(), bool cast = true) {
        std::vector<NumpyColumnPlan> plan = make_numpy_column_plan(this->frame_.columns(), columns);
        const std::vector<size_t> selected_indices = selected_column_indices(plan);
        DataFrameExecutor executor(numpy_worker_count(selected_indices.size()));
        ...
        if (cast) {
            this->frame_.column_parallel_apply(
                executor,
                selected_indices,
                buffers,
                [this](DataFrame<>::column_type column, NumpyColumnBuffer& buffer) {
                    append_numpy_column_filtered(column, buffer, this->deleted_rows_);
                }
            );
            if (needs_string_replay(buffers)) {
                replay_string_columns_from_frame(this->frame_, executor, buffers, this->deleted_rows_);
            }
            finalize_numpy_buffers(buffers);
        }
        else {
            this->frame_.column_parallel_apply(
                ...
                buffers,
                [this](DataFrame<>::column_type column, NumpyColumnBuffer& buffer) {
                    append_numpy_strings_only_filtered(column, buffer, this->deleted_rows_);
                }
            );
        }

...
TEST_CASE("col_pos() Test", "[test_col_pos]") {
    int pos = get_col_pos(
        data_file("/data/real_data/2015_StateDepartment.csv"),
        "Entity Type");
    REQUIRE(pos == 1);
}
...
TEST_CASE("Prevent Column Names From Being Overwritten", "[csv_col_names_overwrite]") {
    ...
    for (auto& format_in : formats) {
        ...
        CSVReader reader(data_file("/data/fake_data/ints_comments.csv"), format_in);
...
TEST_CASE("get_file_info() Test", "[test_file_info]") {
    CSVFileInfo info = get_file_info(data_file("/data/real_data/2009PowerStatus.txt"));
...
TEST_CASE( "Test Read CSV with Header Row", "[read_csv_header]" ) {
    // Header on first row
    const std::string infile = data_file("/data/real_data/2015_StateDepartment.csv");
    ...
    REQUIRE( get_col_names(infile) == col_names );
...
TEST_CASE("Test read_row() CSVField - Power Status", "[read_row_csvf3]") {
    CSVReader reader(data_file("/data/real_data/2009PowerStatus.txt"));
    CSVRow row;
